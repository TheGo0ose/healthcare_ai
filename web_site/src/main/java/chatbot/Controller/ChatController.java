    ...
    public ResponseEntity<Map<String, String>> chat(@RequestBody Map<String, String> payload) {
        ...
        try {
            ...
            if (body == null || !body.containsKey("answer")) { // Изменено на "answer"
                Map<String, String> error = new HashMap<>();
                error.put("error", "Неверный формат ответа от бота");
            ...
            System.err.println("Ошибка при запросе к Flask: " + e.getMessage()); // Логируем точную ошибку
            Map<String, String> error = new HashMap<>();
            error.put("error", "Ошибка соединения с ботом: " + e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(error);
        }
    }
}